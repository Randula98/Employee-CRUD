...
export default function Filter() {
    ...
    useEffect(() => {
        async function getRecords() {
            const response = await fetch(`http://localhost:5000/employee/bank/${bank}/branch/${branch}`);
            if (!response.ok) {
            ...
            <div className="DashContainer">
                <div className="innerContainer rounded">
                    <div className="p-4 mb-4 text-xl text-blue-700 bg-blue-100 rounded-lg dark:bg-gray-400 dark:text-blue-900" role="alert">
                        <span className="font-medium">Employees in {bank} - {branch}</span>
                    </div>
